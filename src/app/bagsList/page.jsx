...
function Page() {
  ...
  const API_URL = `${LOCAL_API_BASE_URL}/api/searchItems/bagsList`; // bagsList API 엔드포인트
  ...
  const fetchSearchResults = async () => {
    ...
    try {
      const response = await axios.get(API_URL, {
        params: {
          keyword: query,
          category: '가방', // bagsList 고정 카테고리
        },
      });

      console.log("API 요청 파라미터:", { keyword: query, category: "가방" });
  ...
  const fetchDefaultItems = async () => {
         ...
         try {
           ...
           console.log("API 요청 파라미터:", { category: "가방" });
           console.log("API 응답 데이터:", response.data);
  ...
  return (
    <>
      <MidCategoryBanner category="bagsList" />
